...
module.exports=class dish{
   ...
   static update2(title,price,category,id)
    ...
    {
        return db.execute('insert into dishes_has_cart(dishes_id,cart_id) values(?,?)',[dishid,cartid])
    }
    static recentid()
    {
        return db.execute('select id from dishes_has_cart order by id desc limit 1')
    }
    static realcart(id,cartid,dishid,extrasid)
    {
        return db.execute('insert into dishes_cart_extras(id,cart_id,dishes_id,extras_id) values(?,?,?,?)',[id,cartid,dishid,extrasid])
    }
    static allcartitems(cartId)
    {
    return db.execute('SELECT distinct id,title,price,url FROM dishes LEFT JOIN dishes_cart_extras ON dishes_cart_extras.dishes_id = dishes.id where dishes_cart_extras.dishes_id=dishes.id and dishes_cart_extras.cart_id=(?)',[cartId])
    }
    static allextras(id)
    {
        return db.execute('SELECT * FROM extras LEFT JOIN dishes_cart_extras ON extras.id = dishes_cart_extras.extras_id where dishes_cart_extras.extras_id=extras.id and cart_id=(?)',[id])
    }
    static total(cartId)
    {
        return db.execute('select * from dishes_has_cart where cart_id=(?)',[cartId])
    }
    static delete22(id)
    {  
